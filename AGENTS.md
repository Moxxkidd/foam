...
包名 `foam`、命令 `foam`/`fm`、env `FOAM_*`、
`src/foam/__init__.py` 的 `__app_name__ = "Foam"`。代码、注释、文档
可正常使用 Foam 之名。

> 历史注记:定名后曾施行「品牌字符串仅许可出现在 pyproject.toml 与
> `__app_name__` 两处、其余一律称『本项目』」的命名纪律,以防碎片化与
> 二次改名成本;该纪律在改名中验证了价值(全仓仅数处手工收口)。
> **2026-08-27 总指挥指示:定名即最终,纪律退役**——历史文档(规格/
> 开发日志/核销表等)中的「本项目」等旧表述为彼时真实记录,不强制回改。
