...
| 门 | 标志 | 门口动作 | 状态 |
|---|---|---|---|
| M1 | WP-04 关闭 | 能力演示:真实 K3 后端 `theform run` 跑通 lab scope 内无害链(补 WP-04 未跑的真实模型 e2e);方向复评 | **已通过(2026-08-21)**:演示补跑通过(exit 0/finished/6 轮,审计链 verify 通过,幻觉纠正真实触发一次且模型以核验响应,零越界零泄漏,证据见 docs/dev-logs/WP-04.md「补跑」节)+ 方向复评**放行**;WP-08/09/10 开闸 |
...
- 2026-08-21 **WP-04 closed**:agent 主环(agent/loop.py:消息状态机 +
  ...
  (fake 后端按 WP-03 实测事件形状合成),有凭据后用 `theform run` 补跑。
