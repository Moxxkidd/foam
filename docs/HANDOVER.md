...
**WP-13 已关闭(2026-08-27);13 个 WP 全部落地,M4 门(发布评审)
随之触发,门口动作待总指挥执行。**v1 候选已按优先级排入下方
「v1 立项清单」节;M3 挂账 9 条原始候选表述保留于下备查。
- 留 M4 门口一并裁决:sqlmap 演示项(挂账⑨,WP-13 规格未含故未做)、
  会话审计(挂账④)与 loot 登记语义(挂账⑥)两项门口定夺、v1 阶段
  开闸与否。
...
## v1 立项清单(2026-08-27 WP-13 编排;M4 门口裁决后生效)

> 来源:WP-13 规格点名 5 项 + M3 挂账 9 条 + 各 WP 遗留在册项,合并去重
> 按优先级排列(括号内为来源编号,见 `docs/known-issues.md`)。优先级口径:
> **P0=安全与审计底线;P1=登记面与正确性;P2=平台能力(规格点名横向
> 能力);P3=演示与维护**。标注「门口」者 M3/M4 未定论,须总指挥裁决后
> 才立项。

**P0 安全与审计底线**

1. **nftables 网络级出口护栏**(规格点名;WP-02 明示排除项)——参数级
   静态护栏绕过面的根治,与「不削弱护栏」红线同向。
2. **会话操作审计 kind + 结构化提示事件落盘**(挂账④⑤并案)——replay
   时间线在纯会话阶段断档的修复;④为**门口裁决项**(M3 未定论,留 M4)。
3. **护栏目标提取器改进**(挂账⑦)——版本号(`4.7p1`/`3.2.8.1`)误判
   越界 IP 的新误报形态根治;误判案例台账常态化。

**P1 登记面与正确性**

4. **state 工具面 add_vuln/add_cred 登记接口**(挂账①)——手工成果与
   未注册工具产出入库,场景一索引四表空的主因修复。
5. **claim-correction 启发式豁免「无新动作声明的总结段」**(挂账②)——
   场景一 4 次/场景二 1 次稳定误触发的治理。
6. **loot 登记语义定夺 + ENGAGEMENT.md 占位文案口径统一**(挂账⑥⑧)——
   ⑥为**门口定夺项**(尺寸快照 vs 终态复核)。
7. **prompts 引导「扫描类命令保持完整输出」**(挂账③)——降低模型
   自截断致解析 no_match 的概率。

**P2 平台能力(规格点名)**

8. **多后端 failover**(规格点名)+ Claude/DeepSeek/GLM/OpenRouter
   实测补足(⑩并案)。
9. **报告模板化**(规格点名;含 PDF/HTML 导出,WP-10 排除项)。
10. **meterpreter 语义层**(规格点名;WP-05 排除项,通道已通,语义化
    管理后续)。
11. **跨 engagement 经验库**(规格点名;WP-06 排除项/远期)。

**P3 演示与维护**

12. **sqlmap 类交互向导实弹演示**(挂账⑨,**门口演示项**)——sqlmap
    友好靶场,objective 不指定工具观察自选;WP-13 规格未含故未做。
13. **工具目录新鲜度机制**(⑪)——静态目录随目标环境漂移(96/137 →
    93/137)的再校准。
14. **TUI `!` shell 前缀**(WP-09 Q4 明示 v1 候选)——须过护栏,不破
    审计。
